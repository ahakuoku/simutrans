...
void convoi_t::init(player_t *player)
{
	...
	*name_and_id = 0;
	...
	next_coupling_index = route_t::INVALID_INDEX;
...
DBG_MESSAGE("convoi_t::finish_rd()","next_stop_index=%d", next_stop_index );
	...
	for(uint8 i=0;  i<anz_vehikel;  i++) {
		// Vehicles image may need to be recalculated according to the convoy's state.
		fahr[i]->calc_image();
	}
	if(  state==LOADING  ) {
...
void convoi_t::calc_acceleration(uint32 delta_t)
{
	...
	if(sum_gesamtweight != 0) {
		...
		delta_v = delta_v * acceleration_magnification / 100;
		...
		case DRIVING:
			{
				...
				// now actually move the units
				sp_soll += (akt_speed*delta_t*speed_magnification/100);
...
//	DBG_MESSAGE("convoi_t::go_alte_richtung()","neu=%i,rwr_neu=%i,alt=%i",neue_richtung_rwr,ribi_t::backward(neue_richtung_rwr),alte_richtung);
	...
	// inspecting represents the last convoy of all coupled convoys.
...
void convoi_t::rdwr(loadsave_t *file)
{
	...
	if(  file->get_OTRP_version()>=40  ) {
		...
		file->rdwr_vector(fetched_fresh_goods, rdwr_goods);
	}
...
void convoi_t::reverse_vehicles_at_halt_if_needed()
{
	// this function is the normal reversing function.
	// this is called when the train arrived at the station if the station is a reversing point.
...
void convoi_t::reverse_vehicles()
{
	uint8 a = 0;
	vehicle_t* reverse;
	uint8 b = get_vehicle_count();
	front()->set_leading(false);
	back()->set_last(false);

	for( ; a<--b; a++) //increment a and decrement b until they meet each other
	{
		reverse = fahr[a]; //put what's in a into swap spacekarte_t::load(
